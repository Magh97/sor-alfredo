        ...
        {navItems.map(({ id, label, icon: Icon, href }) =>
          id === 'profile' ? (
            <button
              key={id}
              onClick={logout}
              className={`flex flex-col items-center justify-center min-h-[48px] min-w-[48px] px-4 py-2 font-['DM_Sans'] font-bold text-xs uppercase tracking-widest transition-colors duration-150 text-[#8B7355]`}
              aria-label={label}
            >
              <Icon size={24} />
